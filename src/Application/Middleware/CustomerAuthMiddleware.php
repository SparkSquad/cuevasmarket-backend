    ...
    public function process(Request $request, RequestHandler $handler): Response
    {
        ...
        if($token) {
            try {
                ...
                if($decoded_array['type'] == User::USER_TYPE_ADMIN || $decoded_array['type'] == User::USER_TYPE_CUSTOMER) {
                    $request = $request->withAttribute('loggedUser', $decoded_array);
                    return $handler->handle($request);
                }
                else {
                    $response = new Psr7Response();
                    $error = new ActionError(ActionError::UNAUTHORIZED, 'User is not a customer.');
